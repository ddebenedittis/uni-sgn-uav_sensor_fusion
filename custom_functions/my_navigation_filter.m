classdef my_navigation_filter < matlab.System
    ...
    properties
        ...
        % UWB unknown
        uwb_unkn_meas_noise = 1e+1              % [m^2]
        uwb_unkn_proc_noise = 1e-2              %
...
%             if norm(obj.state_covariance(16:19,16:19)) < 10^2
                z_g = imu_kalman(obj);
%             else
%                 z_g = imu_kalman_no_pv(obj);
%             end
...
%             else
%                 z_m = magn_kalman_no_pv(obj, magn_meas.');
%             end
...
%         end
        ...
        function z = imu_kalman(obj)
            ...
            gen_kalman_correction(obj, i, R, Hk, z)
        end
        ...
        function z = imu_kalman_no_pv(obj)
            %% Measurement model
...
%             i = [1,12, 16,21];
        ...
        function gen_kalman_correction(obj, i, R, Hk, z)
...
%             i = [1,9, 13,15];
        ...
        % velocity in the measurement model. It provides better performance
        % when the position noise is high
        function z = magn_kalman_no_pv(obj, magn_meas)
            %% Measurement model
...
%             i = [1,9, 13,21];
        ...
        function [beacon_pos] = uwb_unkn_init(obj, dist_meas, index)
            ...
            if size(H, 1) == obj.uwb_unkn_init_threshold
                ...
                obj.uwb_unkn_position(:, index) = x(2:4);
                beacon_pos = x(2:4);
                
%                 obj.uwb_unkn_state_covariance(:,:,index) = 100 * eye(4);
                obj.uwb_unkn_state_covariance(:,:,index) = 100 * eye(3);
        ...
        function [beacon_pos, z] = uwb_unkn_kalman_nav(obj, dist_meas, index)
            ...
            beacon_pos = obj.uwb_unkn_position(:, index);
            ...
            P_b = obj.uwb_unkn_state_covariance(:,:,index);
...
%             P_cor = (eye(18) - K * Hk) * P_pre * (eye(18) - K * Hk).' + K * R * K.';
...
%             P_cor(1:3,1:3) = Gk * P_cor(1:3,1:3) * Gk.';
            ...
            obj.uwb_unkn_position(:, index) = beacon_pos;
            
            obj.uwb_unkn_state_covariance(:,:,index) = P_cor(end-2:end,end-2:end);
        end
...
%                     Hk = Hk(:,10:end);
                    ...
                    % Measurement covariance
                    Rnb = obj.measurement_covariance(17,17);
...
%                     i = [16,20, 21,22];
                    ...
                    gen_kalman_correction(obj, i, Rnb, Hk, z)
                end
            end
        end
